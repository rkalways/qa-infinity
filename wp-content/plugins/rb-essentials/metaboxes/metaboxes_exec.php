                ...
                if( $prop['type'] == 'text' ){
                    ...
                    $out .= '<a href="#" class="'. ( $prop['type'] == 'image' ? 'rb_upload_image' : 'rb_upload_gallery' ) . ( $image ? '' : ' button' ) .'">';
                        if( strpos( rb_get_metabox($key), ',' ) !== false ){
                            ...
                            $out .= $image ? '<img src="'.$image[0].'" />' : esc_html_x('Upload', 'backend', 'seoes');
                        }
                    $out .= '</a>';

                    $out .= '<input type="hidden" name="rb_'.$key.'" id="rb_'.$key.'" value="'.esc_attr($value).'" />';

                    $out .= '<a href="#" class="rb_remove_image" style="display:'.($image ? 'inline-block' : 'none').'">'.esc_html_x('Remove', 'backend', 'seoes').'</a>';
