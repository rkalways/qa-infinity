...
function rb_vc_shortcode_sc_portfolio ( $atts = array(), $content = "" ){
	...
	if( $final_q->have_posts() ):
		ob_start();
			...
			echo "<div id='".$id."' class='".$module_classes."' ".$module_atts.">";
				...
				if( $layout == 'grid_filter' ){
					echo "<ul class='portfolio-filter'>";
						...
						echo "<li class='portfolio-filter-trigger active' data-value='all'>".esc_html_x('All', 'frontend', 'seoes')."</li>";
						foreach( $terms as $term ){
													...
													echo '<div class="advanced_info">';
														...
														echo '<div class="advanced_details">';
															echo '<div class="advanced_detail">';
																echo '<p class="label">'.esc_html_x('Date', 'frontend', 'seoes').'</p>';
																echo get_the_date();
															echo '</div>';

															if( !empty($author) ){
																echo '<div class="advanced_detail">';
																	echo '<p class="label">'.esc_html_x('Author', 'frontend', 'seoes').'</p>';
															...
															if( !empty($client) ){
																echo '<div class="advanced_detail">';
																	echo '<p class="label">'.esc_html_x('Client', 'frontend', 'seoes').'</p>';
			...
			if( !$is_carousel ) seoes__pagination( $final_q, $total_items_count, $items_pp, $pagination, $pages_quantity );
...
function rb_portfolio_scripts() {
	...
	wp_register_script( 'rb-portfolio', plugin_dir_url( __FILE__ ) . '../assets/js/rb-portfolio.js', array('jquery'), SEOES__VERSION, true );
	wp_register_script( 'jquery-easing', plugin_dir_url( __FILE__ ) . '../assets/js/jquery.easing.js', array('jquery'), '1.3.0', true );
	wp_register_script( 'rb-parallax-scroll', plugin_dir_url( __FILE__ ) . '../assets/js/jquery.parallax-scroll.js', array( 'jquery-easing' ), '1.0.0', true );
