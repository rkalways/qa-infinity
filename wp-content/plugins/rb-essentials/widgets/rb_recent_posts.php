...
class RB_Recent_Posts extends WP_Widget {
	public function __construct() {
		$widget_options = array( 
		  ...
		  'description' => esc_html_x('Displays custom "RB Recent Posts" widget', 'Widget RB-Recent-Posts', 'seoes'),
		);
	...
	public function form( $instance ) {
		$title = !empty( $instance['title'] ) ? $instance['title'] : esc_html_x( 'New widget', 'Widget RB-Recent-Posts', 'seoes' ); 
		$posts_count = !empty( $instance['posts_count'] ) ? $instance['posts_count'] : 3;
		?>
		
		<div class='widget-row'>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>">
				<?php echo esc_html_x( 'Title:', 'Widget RB-Recent-Posts', 'seoes' ); ?>
			</label>
		...
		<div class='widget-row'>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts_count' ) ); ?>">
				<?php echo esc_html_x( 'Posts count:', 'Widget RB-Recent-Posts', 'seoes' ); ?>
			</label>
			<div class="widget-inner-content">
