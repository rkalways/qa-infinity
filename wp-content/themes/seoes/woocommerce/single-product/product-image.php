...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;" <?php echo esc_attr($data); ?>>
	<figure class="woocommerce-product-gallery__wrapper">
		...
		if ( $product->get_image_id() ) {
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="no_lazy_load wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'seoes' ) );
			$html .= '</div>';
		}
